...
在理解为什么 `sleep` 函数需要传入一个锁之前，我们首先探讨如果实现一个不需要传入锁的 `sleep` 函数会发生什么问题，这样的实现往往会导致 **“lost wakeup”（丢失唤醒）** 问题。

#### Lost Wakeup 问题的形成

假设我们有一个不带锁的 `sleep` 函数实现，称为 `broken_sleep`。这个函数通过设置进程的状态为 `SLEEPING`，并记录 `sleep channel`，最后调用 `switch` 来出让 CPU。但这样的实现会导致以下问题：

1. **未能及时进入 SLEEPING 状态**：在没有锁的保护下，`uartwrite` 函数可能刚好在执行 `broken_sleep` 之前释放了锁，导致 CPU 立即可以处理中断。
2. **中断处理程序的提前执行**：此时中断处理程序（如 `uartintr`）获取到锁并发现 UART 硬件空闲，于是将 `tx_done` 标志置为 1，并调用 `wakeup` 函数。
3. **错过唤醒**：因为 `uartwrite` 还没来得及进入 `SLEEPING` 状态，`wakeup` 函数无法找到任何在对应 `channel` 上等待的线程，这样当前的 `wakeup` 就被“丢失”了。
4. **无限期等待**：当 `uartwrite` 最终调用 `broken_sleep` 后，由于错过了唤醒，它将永远不会被唤醒，导致系统卡死。

这种在 `sleep` 调用前释放锁后立即遇到中断的情况，便形成了 **lost wakeup** 问题。

#### 示例：在 UART 驱动中使用 `broken_sleep`

在 UART 驱动的 `uartwrite` 函数中，为每个要写入的字符调用 `sleep` 并等待 `tx_done` 被中断程序置为 1。这种实现中，只有在调用 `sleep` 前释放了锁，中断程序才能获取锁并设置 `tx_done` 为 1，然后唤醒线程。但由于 `sleep` 在不加锁的情况下无法正确处理这些竞争条件，导致出现 lost wakeup 问题。

#### 解决方法：传入锁的 `sleep` 实现

在 `sleep` 函数的正确实现中，增加了锁的传入参数。这样可以保证线程在以下步骤中避免 lost wakeup：

1. **获取锁**：在进入 `sleep` 函数时锁已经持有。
2. **状态设置**：将线程的状态设置为 `SLEEPING`，并将 `sleep channel` 记录到线程结构中。
3. **释放锁并调用 `switch`**：在确保线程已经进入 `SLEEPING` 状态后才释放锁，避免中断提前处理。

通过这种方式，`sleep` 和 `wakeup` 不会因 race condition 而出现丢失唤醒的问题。
